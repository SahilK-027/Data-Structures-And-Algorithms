class Solution {
private:
    int solveMemoization(vector<int>& slices, int s, int e, int n, vector<vector<int>>&dp){
        // Base case if slices_we_can_eat == 0 or s > e
        if(n == 0 || s > e)
            return 0;
        if(dp[s][n] != -1) return dp[s][n];
        int include = slices[s] + solveMemoization(slices, s +2,e, n-1, dp);
        int exclude = solveMemoization(slices, s+1, e , n, dp);
...
public:
    int maxSizeSlices(vector<int>& slices) {
        ...
        int option1 = solveMemoization(slices, 0, size-2, slices_we_can_eat, dp1);
        int option2 = solveMemoization(slices, 1, size-1, slices_we_can_eat, dp2);
